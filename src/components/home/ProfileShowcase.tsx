...
const SHOWCASES: Showcase[] = [
  {
    id: "free",
    tabKey: "home.showcase.tab.free",
    path: "u/studio",
    nameKey: "home.showcase.free.name",
    taglineKey: "home.showcase.free.tagline",
    badge: "shield",
    badgeLabelKey: "home.showcase.free.badgeLabel",
    ...
    linkKeys: [
      "home.showcase.free.link.newsletter",
      "home.showcase.free.link.calbooking",
      "home.showcase.free.link.instagram",
      "home.showcase.free.link.pixelfed",
    ],
    widgetKey: "home.showcase.free.widget",
  },
  {
    id: "pro",
    tabKey: "home.showcase.tab.pro",
    path: "jdelplanche",
    nameKey: "home.showcase.pro.name",
    taglineKey: "home.showcase.pro.tagline",
    badge: "verified",
    badgeLabelKey: "home.showcase.pro.badgeLabel",
    ...
    linkKeys: [
      "home.showcase.pro.link.matrix",
      "home.showcase.pro.link.github",
      "home.showcase.pro.link.mastodon",
      "home.showcase.pro.link.vcard",
    ],
    widgetKey: "home.showcase.pro.widget",
  },
];
...
export function ProfileShowcase() {
  ...
  const theme = dark ? item.dark : item.light;
    ...
    <section className="mx-auto w-full max-w-5xl px-4 py-8 md:py-14">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-center gap-2 md:mt-4">
        ...
        >
          ...
          {dark ? t("home.showcase.darkTheme") : t("home.showcase.lightTheme")}
        </button>
        <button
          type="button"
          ...
          className="inline-flex min-h-9 items-center gap-1.5 rounded-full border border-border px-3 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent"
        >
          <QrCode className="h-3.5 w-3.5" aria-hidden /> {showQr ? t("home.showcase.hideQr") : t("home.showcase.showQr")}
        </button>
        <a
          href={url}
          target="_blank"
        ...
        >
          {t("home.showcase.openLiveProfile")} <ArrowUpRight className="h-3.5 w-3.5" aria-hidden />
        </a>
      </div>
      ...
      >
        ...
        >
          {t(item.nameKey).slice(0, 2).toUpperCase()}
        </div>
        <h3 className="mt-3 flex items-center justify-center gap-1.5 font-serif text-lg md:text-xl">
          {t(item.nameKey)}
          {item.badge === "verified" ? (
            <BadgeCheck className="h-4 w-4 text-sky-400" aria-label={t(item.badgeLabelKey)} />
          ) : (
            <Shield className="h-4 w-4 text-emerald-500" aria-label={t(item.badgeLabelKey)} />
          )}
        </h3>
        <p className="mt-1 font-mono text-xs" style={{ color: theme.muted }}>
          rout.be/{item.path}
        </p>
        ...
        <p className="mt-3 text-[11px] uppercase tracking-widest md:mt-4" style={{ color: theme.muted }}>
          {t(item.widgetKey)}
        </p>
